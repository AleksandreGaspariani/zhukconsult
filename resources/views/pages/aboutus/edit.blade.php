...
@section('content')
    ...
    <div class='w-100 d-flex justify-content-center' style="min-height: 50vh;padding: 10%">
        <form action="/about_us/update/post/{{$postId}}" method="post" enctype="multipart/form-data" id="post-form">
            @csrf
            <input type="input" name="image_default" class="d-none" value="{{$image}}">
            <input type="input" name="post_id" class="d-none" value="{{$postId}}">
            <div class="d-flex flex-column">
                <textarea name="post_text" id="post_text" class="tinymce">
                    {{$text}}
                </textarea>

                <div class="form-floating my-5">
                    <input class="form-control" placeholder="Leave a comment here" value="{{$email}}" name="email"></input>
                    <label for="floatingTextarea">Input your Email here</label>
                </div>

                <div class="d-flex flex-column justify-content-center align-items-center w-100 text-center">
                    <img src="{{asset('uploads/aboutUsImgs/'.$image)}}" alt="" width="350px" height="auto">
